...
fn add_core_tool_sources(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    registry.add(AccountRateLimitsHandler);
...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    registry.add(ContextUsageHandler);
    crate::tools::context_tools_plan::add_to(registry);
    ...
    if turn_context.config.include_skill_instructions
        && !skills_snapshot
            ...
            .is_empty()
    {
        registry.add(UseSkillHandler::from_snapshot(skills_snapshot));
    }
