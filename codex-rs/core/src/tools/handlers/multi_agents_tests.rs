...
#[tokio::test]
async fn multi_agent_v2_spawn_accepts_available_model_with_different_backend_metadata() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        task_name: String,
    }

    let (mut session, mut turn, _receiver) = make_session_and_context_with_rx().await;
    let mut config = (*turn.config).clone();
    config
        .features
        .enable(Feature::MultiAgentV2)
    ...
    let output = SpawnAgentHandlerV2::default()
        .handle(invocation(
            ...
            function_payload(json!({
                "message": "inspect this repo",
                "task_name": "cross_backend_model",
                "model": "gpt-5.4",
                "fork_turns": "none"
            })),
        ))
        .await
        .expect("an available model should not be rejected by backend metadata");
...
async fn multi_agent_v2_send_message_rejects_interrupt_parameter() {
    ...
    assert!(message.starts_with(
        "failed to parse function arguments: unknown field `interrupt`, expected one of `target`, `message`, `in_reply_to`"
    ));
