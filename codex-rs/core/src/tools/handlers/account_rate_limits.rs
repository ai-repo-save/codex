    ...
    fn new(response: AccountRateLimitsResponse) -> Result<Self, FunctionCallError> {
        let text = serde_json::to_string(&response).map_err(|err| {
            FunctionCallError::Fatal(format!(
                "failed to serialize {GET_ACCOUNT_RATE_LIMITS_TOOL_NAME} response: {err}"
            ))
        })?;
    ...
    async fn handle_call(
        ...
        let client = BackendClient::from_auth(turn.config.chatgpt_base_url.clone(), &auth)
            .map_err(|err| {
                FunctionCallError::RespondToModel(format!(
                    "failed to construct account rate limits client: {err}"
                ))
            })?;
        ...
        if rate_limits.is_empty() {
            return Err(FunctionCallError::RespondToModel(
                "failed to fetch account rate limits: no rate-limit snapshots returned".to_string(),
            ));
        }
