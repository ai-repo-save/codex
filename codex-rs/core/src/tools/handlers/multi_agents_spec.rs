...
pub fn create_spawn_agent_tool_v2(options: SpawnAgentToolOptions) -> ToolSpec {
    ...
    let inherited_model_guidance =
        (!options.hide_agent_type_model_reasoning).then_some(SPAWN_AGENT_INHERITED_MODEL_GUIDANCE);
    let mut properties =
        spawn_agent_common_properties_v2(&options.agent_type_description, options.encrypt_messages);
    if options.hide_agent_type_model_reasoning {
        hide_spawn_agent_metadata_options(&mut properties);
    }
    properties.insert(
...
pub fn create_send_message_tool(options: MessageToolOptions) -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target".to_string(),
            JsonSchema::string(Some(
    ...
    let properties = BTreeMap::from([
        ...
        (
            ...
            JsonSchema::number(Some(
                "Maximum time to wait. Defaults to 240000 ms in authoritative mode, bounded by configured limits, and the configured wait timeout in consult mode."
                    .to_string(),
            )),
        ),
        (
...
pub fn create_followup_task_tool(options: MessageToolOptions) -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target".to_string(),
            JsonSchema::string(Some(
...
fn spawn_agent_common_properties_v2(
    ...
    BTreeMap::from([
        (
            "message".to_string(),
            message_schema("Initial plain-text task for the new agent.", encrypt_messages),
        ),
        (
            "agent_type".to_string(),
            JsonSchema::string(Some(agent_type_description.to_string())),
...
fn message_schema(description: &str, encrypt_messages: bool) -> JsonSchema {
...
{usage_hint_text}"#
    ...
    if let Some(max_concurrent_threads_per_session) = max_concurrent_threads_per_session {
        return format!(
            r#"
        {tool_description}
This session is configured with {max_concurrent_threads_per_session} concurrently open agent threads."#
        );
    }
    tool_description
}
