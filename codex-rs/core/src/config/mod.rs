...
pub struct MultiAgentV2Config {
    ...
    #[serde(skip)]
    pub multi_agent_mode_explicitly_configured: bool,
...
impl MultiAgentV2Config {
    fn defaults_for_max_concurrency(max_concurrent_threads_per_session: usize) -> Self {
        Self {
            ...
            multi_agent_mode_explicitly_configured: false,
...
impl ConfigBuilder {
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        {
            let export_dir =
                config_lock_settings.and_then(|config_lock| config_lock.export_dir.clone());
            ...
            .await?;
            config.config_lock_export_dir = export_dir;
...
fn resolve_multi_agent_v2_config(config_toml: &ConfigToml) -> MultiAgentV2Config {
    ...
    let encrypt_messages = base
        .and_then(|config| config.encrypt_messages)
        .unwrap_or(default.encrypt_messages);
