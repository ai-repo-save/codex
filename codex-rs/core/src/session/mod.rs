...
impl Session {
    ...
    pub(crate) async fn consult_snapshot(&self) -> ConsultSessionSnapshot {
        ...
        let item_count = state.history.annotated_items().len();
    ...
    pub(crate) async fn refresh_hooks(&self, config: Arc<Config>) {
        ...
        if Arc::ptr_eq(
            ...
            match self.hooks().reconfigured(hooks_config) {
                Ok(hooks) => self.services.hooks.store(Arc::new(hooks)),
                Err(err) => warn!("failed to refresh required managed hooks: {err}"),
            }
        }
    }
