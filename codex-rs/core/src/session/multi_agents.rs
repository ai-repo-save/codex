...
pub(crate) fn effective_multi_agent_mode(turn_context: &TurnContext) -> Option<MultiAgentMode> {
    ...
    // Configured values win over catalog defaults. A configured hint, including an empty
    // string, defines a custom policy; otherwise an explicitly configured built-in mode wins.
    let multi_agent_mode = if let Some(hint_text) = config.multi_agent_mode_hint_text.as_ref() {
        MultiAgentMode::Custom(hint_text.clone())
    } else if config.multi_agent_mode_explicitly_configured {
        config.multi_agent_mode.clone()
    } else if let Some(hint_text) = catalog_mode.and_then(|mode| mode.hint_text.as_ref()) {
        MultiAgentMode::Custom(hint_text.clone())
    } else {
