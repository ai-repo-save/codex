...
use crate::session::context_lifecycle;
...
async fn drain_in_flight(
    ...
    context_lifecycle::rewind_must_be_sole_tool_call(&outputs)?;
    ...
    for output in outputs {
        match output {
            ...
            other => {
                let handled = context_lifecycle::apply_context_lifecycle_tool_output(
                    &sess,
                    &turn_context,
                    other,
                )
                ...
                if !handled {
                    return Err(CodexErr::Fatal(
                        "unhandled in-flight tool output after context lifecycle dispatch"
                            .to_string(),
                    ));
                }
            }
