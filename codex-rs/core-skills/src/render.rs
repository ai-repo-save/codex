...
- How to use a skill (progressive disclosure):
  1) After deciding to use a skill, you must call the `use_skill` tool with its listed name before taking task actions. Do not read `SKILL.md` directly unless `use_skill` is unavailable or fails.
  2) If `use_skill` is unavailable or fails, read the source directly using the same access mechanism: open a `file` path, use the owning environment filesystem for an `environment resource`, call `skills.list`/`skills.read` for an `orchestrator resource`, or use the provider mechanism for a `custom resource`. If a read is truncated or paginated, continue until EOF.
  ...
  5) For filesystem-backed skills, prefer running or patching provided scripts instead of retyping large code blocks. For orchestrator skills, use `skills.read` and the available tools; do not invent a local path.
