...
- How to use a skill (progressive disclosure):
  1) After deciding to use a skill, read its `SKILL.md` completely before taking task actions. For a `file` entry, call `use_skill` with its listed name; if `use_skill` is unavailable or fails, open the listed path directly. For an `environment resource`, use the exact authority, package, and `main_resource` from the selected skill's `resource_access` metadata when present; otherwise call `skills.list` with `{"authority":{"kind":"executor"}}`. For an `orchestrator resource`, call `skills.list` with `{"authority":{"kind":"orchestrator"}}`. Select the matching package and pass its exact authority, package, and `main_resource` to `skills.read`. Follow `next_cursor`; if a read is paginated, continue until EOF.
