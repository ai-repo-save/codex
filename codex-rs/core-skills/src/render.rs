...
- How to use a skill (progressive disclosure):
  1) After deciding to use a skill, read its `SKILL.md` completely before taking task actions. For a `file` entry, you must call the `use_skill` tool with its listed name. Do not read `SKILL.md` directly unless `use_skill` is unavailable or fails; if you fall back to a direct read, open the listed path and continue until EOF. For an `environment resource`, use the exact authority, package, and `main_resource` from the selected skill's `resource_access` metadata when present; otherwise call `skills.list` with `{"authority":{"kind":"executor"}}`. For an `orchestrator resource`, call `skills.list` with `{"authority":{"kind":"orchestrator"}}`. Select the matching package and pass its exact authority, package, and `main_resource` to `skills.read`. Follow `next_cursor`; if a read is paginated, continue until EOF.
  2) When `SKILL.md` references another resource, use the same access mechanism. Resolve relative paths against a filesystem-backed skill directory. Resolve relative references beneath an executor skill's returned package and call `skills.read` with the same authority and package. For orchestrator skills, pass the exact referenced resource identifier with the same authority and package to `skills.read`; do not treat `skill://` identifiers as filesystem paths.
