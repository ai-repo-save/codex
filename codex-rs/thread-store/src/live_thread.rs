...
impl LiveThread {
    ...
    /// Loads canonical replay history, bypassing rebuildable query projections.
    pub async fn load_canonical_history(
        &self,
        include_archived: bool,
    ) -> ThreadStoreResult<StoredThreadHistory> {
        self.thread_store
            .load_canonical_history(LoadThreadHistoryParams {
                thread_id: self.thread_id,
                include_archived,
            })
            .await
