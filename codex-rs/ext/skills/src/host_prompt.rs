...
use codex_skills::strip_skill_frontmatter;
...
impl HostSkillsSnapshot {
    /// Loads one host skill for an explicit `use_skill` invocation.
    pub async fn load_skill_prompt(
        &self,
        skill: &SkillMetadata,
    ) -> Result<Box<dyn ContextualUserFragment + Send>, String> {
        let contents = self.read_skill_text(skill).await.map_err(|err| {
            format!(
        ...
        Ok(Box::new(SkillInstructions::from_host_skill(
            skill.name.clone(),
            skill.path_to_skills_md.to_string_lossy().into_owned(),
            strip_skill_frontmatter(&contents).to_string(),
        )))
    }
