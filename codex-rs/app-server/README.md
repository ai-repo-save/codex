...
- `turn/outputThroughput/updated` — `{ threadId, turnId, active, outputTokens, activeDurationMs, tokensPerSecond }` marks output generation as active or complete. After each completed sampling request, its metrics are the current turn-wide `sum(outputTokens)`, `sum(activeDurationMs)`, and resulting `tokensPerSecond`; clients must display the latest notification rather than accumulating it again. All three metrics are `null` for an active sample and remain `null` for the rest of a turn when any completed sample lacks usage, first visible output, or duration. This notification is transient and is not persisted in rollout history.
...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, operation, outcome, model}` describing a sub-agent lifecycle action. `kind` is `started`, `interacted`, or `interrupted`; `operation` is `sendMessage`, `followupTask`, `parentReply`, or `inspectAgent` when known; `outcome` is `succeeded` or `failed` when known. `operation` and `outcome` are `null` for older rollout entries. The item carries activity metadata only and does not include request or response body content. `model` contains the effective model for `started` and is `null` for the other actions and older rollout entries.
