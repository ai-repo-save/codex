...
- `configRequirements/read` — fetch loaded requirements constraints from `requirements.toml` and/or MDM (or `null` if none are configured), including allow-lists (`allowedApprovalPolicies`, `allowedSandboxModes`, `allowedWebSearchModes`), the layered permission-profile allow map (`allowedPermissionProfiles`), the managed permission-profile default (`defaultPermissions`), lifecycle hook lockdown (`allowManagedHooksOnly`), remote-control policy (`allowRemoteControl`; `false` force-disables remote control while `true` or `null` preserves existing behavior), computer use policy (`computerUse`), pinned feature values (`featureRequirements`), managed lifecycle hooks (`hooks`, including complete raw Prompt definitions with nullable `reasoningEffort` and `filter`), `enforceResidency`, managed new-thread defaults (`models.newThread.model`, `models.newThread.modelReasoningEffort`, and `models.newThread.serviceTier`), and `network` constraints such as canonical domain/socket permissions plus `managedAllowedDomainsOnly` and `dangerFullAccessDenylistOnly`.
...
The app-server streams JSON-RPC notifications while a turn is running. Each turn emits `turn/started` when it begins running and ends with `turn/completed` (final `turn` status). Token usage and output-throughput events stream separately via `thread/tokenUsage/updated` and `turn/outputThroughput/updated`. Clients subscribe to the events they care about, rendering each item incrementally as updates arrive. The per-item lifecycle is always: `item/started` → zero or more item-specific deltas → `item/completed`.
...
- `turn/outputThroughput/updated` — `{ threadId, turnId, active, outputTokens, activeDurationMs, tokensPerSecond }` reports core-calculated output throughput for the current turn. While `active` is `true`, `tokensPerSecond` is the current approximate rate when enough output has arrived and may otherwise be `null`; `outputTokens` and `activeDurationMs` are `null`. When `active` is `false`, `tokensPerSecond`, `outputTokens`, and `activeDurationMs` are the exact accumulated values when available, or all are `null` when exact accounting is unavailable. Clients display the latest notification as-is and do not estimate, accumulate, or recompute these values. This notification is runtime-only and is not reconstructed from rollout history.
