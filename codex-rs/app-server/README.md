...
- `collaborationMode/list` — list available collaboration mode presets (experimental, no pagination). Built-in Default and Plan presets do not select a model. Default does not override reasoning effort, and Plan selects medium reasoning effort. This response omits built-in developer instructions; clients should either pass `settings.developer_instructions: null` when setting a mode to use Codex's built-in instructions, or provide their own instructions explicitly.
...
- `config/read` — fetch the runtime-effective config after resolving config layering and managed requirements, including opaque `desktop` values and Prompt hook `filter` definitions stored in `config.toml`. When configured, the `packagedDefaults` layer has the lowest precedence.
