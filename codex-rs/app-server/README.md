...
- `thread/reset-context` — experimental; create a new subscribed thread from the active model-visible context of a loaded, idle source thread. It accepts configuration overrides for the new thread and emits `thread/started`; an active, unloaded, or system-error source thread is rejected.
- `thread/start`, `thread/resume`, `thread/fork`, and `thread/reset-context` responses include the legacy `sandbox` compatibility projection. `instructionSources` lists loaded instruction files using each source environment's native absolute path syntax, including files loaded from remote environments. Experimental clients can read `runtimeWorkspaceRoots` for the thread-scoped runtime roots and `activePermissionProfile` for the named or implicit built-in profile identity/provenance when known. The deprecated experimental `multiAgentMode` field on start, resume, and fork responses, and the corresponding thread setting, always reports `explicitRequestOnly`; Ultra reasoning effort is the source of proactive multi-agent behavior.
...
- `hooks/list` — list discovered hooks for one or more `cwd` values, including each Prompt handler's raw nullable `reasoningEffort` and normalized nullable `filter` command.
...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values and Prompt hook `filter` definitions stored in `config.toml`.
...
- `configRequirements/read` — fetch loaded requirements constraints from `requirements.toml` and/or MDM (or `null` if none are configured), including allow-lists (`allowedApprovalPolicies`, `allowedSandboxModes`, `allowedWebSearchModes`), the layered permission-profile allow map (`allowedPermissionProfiles`), the managed permission-profile default (`defaultPermissions`), lifecycle hook lockdown (`allowManagedHooksOnly`), remote-control policy (`allowRemoteControl`; `false` force-disables remote control while `true` or `null` preserves existing behavior), computer use policy (`computerUse`), pinned feature values (`featureRequirements`), managed lifecycle hooks (`hooks`, including complete raw Prompt definitions with nullable `reasoningEffort` and `filter` plus each command handler's optional `additionalContextLimit`), `enforceResidency`, managed new-thread defaults (`models.newThread.model`, `models.newThread.modelReasoningEffort`, and `models.newThread.serviceTier`), and `network` constraints such as canonical domain/socket permissions plus `managedAllowedDomainsOnly` and `dangerFullAccessDenylistOnly`.
...
### Example: Reset a thread's context (experimental)

Enable `capabilities.experimentalApi` during initialization, then call `thread/reset-context` for a loaded, idle source thread. The server creates a new thread from the source thread's current model-visible context; it does not mutate the source thread. The request requires `threadId` and accepts optional `model`, `modelProvider`, `serviceTier`, `cwd`, `runtimeWorkspaceRoots`, `approvalPolicy`, `approvalsReviewer`, `sandbox`, `permissions`, `config`, `baseInstructions`, `developerInstructions`, `ephemeral`, and `threadSource` overrides. `serviceTier: null` explicitly clears the service tier; omit it to leave that override unset. `runtimeWorkspaceRoots` is experimental and resolves relative paths against the app-server process cwd. `sandbox` and `permissions` cannot be combined.

```json
{ "method": "thread/reset-context", "id": 14, "params": {
    "threadId": "thr_123",
    "model": "gpt-5.1-codex",
    "cwd": "/Users/me/project",
    "permissions": ":workspace"
...
{ "id": 14, "result": {
    "thread": { "id": "thr_456", "sessionId": "thr_456", "…": "…" },
    "model": "gpt-5.1-codex",
    "modelProvider": "openai",
    "serviceTier": null,
    "cwd": "/Users/me/project",
    ...
    "sandbox": { "type": "workspaceWrite", "writableRoots": ["/Users/me/project"], "networkAccess": false },
...
The response always includes `thread`, `model`, `modelProvider`, `serviceTier`, `cwd`, `instructionSources`, `approvalPolicy`, `approvalsReviewer`, `sandbox`, and `reasoningEffort`. Experimental clients also receive `runtimeWorkspaceRoots` and, when known, `activePermissionProfile`. When the source has persisted token usage, the server sends `thread/tokenUsage/updated` for the new thread after the response and before `thread/started`. If the source has an unfinished goal, the new thread receives an active copy and the source goal becomes paused; completed goals are not copied.
...
The app-server streams JSON-RPC notifications while a turn is running. Each turn emits `turn/started` when it begins running and ends with `turn/completed` (final `turn` status). Token usage streams separately via `thread/tokenUsage/updated`. Clients subscribe to the events they care about, rendering each item incrementally as updates arrive. The per-item lifecycle is always: `item/started` → zero or more item-specific deltas → `item/completed`.
...
- `collabToolCall` — `{id, tool, status, senderThreadId, receiverThreadIds, prompt?, model?, reasoningEffort?, serviceTier?, contextInheritance, mode?, snapshotRevision?, agentsStates}` describing collaboration tool calls. Wire values for `tool` are `spawnAgent`, `sendInput`, `askParent`, `resumeAgent`, `wait`, and `closeAgent`; `status` is `inProgress`, `completed`, or `failed`. `contextInheritance` is `null` when no spawn context applies, or `{type: "none"}`, `{type: "full"}`, or `{type: "lastNTurns", turns}` for a spawned agent's effective parent-context inheritance. For `askParent`, the direct parent appears in `receiverThreadIds`.

  `askParent.mode` is `authoritative` by default. An authoritative request is delivered to the live direct parent and waits for a correlated `sendMessage` reply; the parent uses the authoritative request's `requestId` as `inReplyTo`. `mode: "consult"` runs a one-shot responder from a parent snapshot identified by the opaque `snapshotRevision`. Consult mode is advisory and may be stale by definition; the model-visible `ask_parent` tool result uses snake_case fields, including `advisory`, `snapshot_may_be_stale: true`, and `status: "requires_authoritative_parent"` when a live parent decision is required. These result fields are not fields on `collabToolCall`. A consult `requestId` is for observability only and cannot be used as `inReplyTo`; consult does not wake, resume, or send a request to the live parent. For authoritative requests, read `agentsStates[parentThreadId].status`: `completed` carries the correlated answer in `message`, `interrupted` means the request timed out, and `notFound` means the parent became unavailable.
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, operation, outcome, model, reasoningEffort, serviceTier, contextInheritance}` describing a sub-agent lifecycle action. `kind` is `started`, `interacted`, or `interrupted`; `operation` is `sendMessage`, `followupTask`, `parentReply`, or `inspectAgent` when known; `outcome` is `succeeded` or `failed` when known. `contextInheritance` is `null` when no spawn context applies, or `{type: "none"}`, `{type: "full"}`, or `{type: "lastNTurns", turns}` for a started agent's effective parent-context inheritance. `operation`, `outcome`, `reasoningEffort`, and `serviceTier` are `null` for older rollout entries. The item carries activity metadata only and does not include request or response body content. `model`, `reasoningEffort`, and `serviceTier` contain the effective settings for `started` and are `null` for the other actions and older rollout entries.
- `memoryMutation` — `{id, action, scope, status, title, path, preview}` describes a visible memory write or deletion. `action` is `write` or `delete`; `scope` is `global`, `session`, or `project`; and `status` is `inProgress`, `succeeded`, or `failed`. `title`, `path`, and `preview` are nullable. Memory mutations emit `item/started` and `item/completed` with the same item id.
- `agentMailboxAction` — `{id, status, action}` describes a durable agent-mailbox `send` or `read`; `status` is `inProgress`, `succeeded`, or `failed`. Send actions carry `{type: "send", target, recipient, category, preview}`, while read actions carry `{type: "read", sender, category, limit, messages}`. Plaintext previews are normalized and capped at 160 graphemes; encrypted messages expose `{type: "encrypted"}` and never expose their body. Completed actions persist in the current thread history and emit `item/started` and `item/completed` with the same item id.
...
- `contextAnchorSaved` — `{id, anchorId, label, historyBoundary, createdAt}` emitted when codex saves a rewind anchor for the current thread. `label` is required and nullable.
  ...
  "result": {
    "data": [{
      ...
      "hooks": [{
        ...
        "matcher": "Bash",
        "command": null,
        "prompt": "Review this tool call and return whether it is safe: $$ARGUMENTS",
        "model": null,
        "filter": {
          "command": "uv run --script /Users/me/.codex/hooks/filter-search-safety.py",
          "commandWindows": null,
          "timeout": 5
        },
