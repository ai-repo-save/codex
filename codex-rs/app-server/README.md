...
- `collabToolCall` — `{id, tool, status, senderThreadId, receiverThreadIds, prompt?, model?, reasoningEffort?, agentsStates}` describing collaboration tool calls. Wire values for `tool` are `spawnAgent`, `sendInput`, `askParent`, `resumeAgent`, `wait`, and `closeAgent`; `status` is `inProgress`, `completed`, or `failed`. For `askParent`, the direct parent appears in `receiverThreadIds`. On completion, read `agentsStates[parentThreadId].status`: `completed` carries the correlated answer in `message`, `interrupted` means the request timed out, and `notFound` means the parent became unavailable.
