...
- `collabToolCall` — `{id, tool, status, senderThreadId, receiverThreadIds, prompt?, model?, reasoningEffort?, mode?, snapshotRevision?, agentsStates}` describing collaboration tool calls. Wire values for `tool` are `spawnAgent`, `sendInput`, `askParent`, `resumeAgent`, `wait`, and `closeAgent`; `status` is `inProgress`, `completed`, or `failed`. For `askParent`, the direct parent appears in `receiverThreadIds`.
...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, operation, outcome, model, reasoningEffort}` describing a sub-agent lifecycle action. `kind` is `started`, `interacted`, or `interrupted`; `operation` is `sendMessage`, `followupTask`, `parentReply`, or `inspectAgent` when known; `outcome` is `succeeded` or `failed` when known. `operation`, `outcome`, and `reasoningEffort` are `null` for older rollout entries. The item carries activity metadata only and does not include request or response body content. `model` and `reasoningEffort` contain the effective settings for `started` and are `null` for the other actions and older rollout entries.
