...
impl ThreadRequestProcessor {
    ...
    async fn build_thread_fork_response_with_history(
        ...
        let source_thread = self
            ...
            .await?;
        let paginated_source = source_history_override.is_none()
            && matches!(source_thread.history_mode, ThreadHistoryMode::Paginated);
        ...
        let source_history_items = if let Some(history) = source_history_override {
            Arc::new(history.into_iter().map(RolloutItem::ResponseItem).collect())
    ...
    async fn thread_reset_context_inner(
        ...
        let context_history = source_thread.current_context_history().await;
