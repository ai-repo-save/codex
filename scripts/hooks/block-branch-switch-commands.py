...
"""Block `git switch` / `git checkout` branch changes."""
...
def main() -> int:
    ...
    if reason := blocked_command_reason(request.command):
        print(
            json.dumps(
                block_decision(request.command, reason),
                ensure_ascii=False,
            )
        )
    return 0
        ...
        if command_position:
            ...
            if executable in SHELL_EXECUTABLES:
                ...
                if script is None:
                    return False
                try:
                    inner_words = shlex.split(script, comments=False, posix=True)
                except ValueError:
                    return False
                return shell_words_switch_branch(strip_env_prefix(inner_words))
